...
rm(list=c("OK2Install","libs","ipak"))
...
# We need a better name for our response variable
names(d.f)[length(d.f)] <- "response"
...
d.f = within(d.f,{
  ...
  response =  factor(response, levels = 0:1, labels = c("No Default","Default"))
})
